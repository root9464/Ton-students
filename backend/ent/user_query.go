...
func (uq *UserQuery) QueryServices() *ServiceQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withServices: uq.withServices.Clone(),
...
func (uq *UserQuery) WithServices(opts ...func(*ServiceQuery)) *UserQuery {
	query := (&ServiceClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withServices = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withServices; query != nil {
		if err := uq.loadServices(ctx, query, nodes,
...
func (uq *UserQuery) loadServices(ctx context.Context, query *ServiceQuery, nodes []*User, init func(*User), assign func(*User, *Service)) error {
	...
	for _, n := range neighbors {
		fk := n.user_name
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_name" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_name" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
