...
func (tq *TagsQuery) QueryServiceTags() *ServiceTagQuery {
	query := (&ServiceTagClient{config: tq.config}).Query()
...
func (tq *TagsQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (tq *TagsQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (tq *TagsQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (tq *TagsQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (tq *TagsQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (tq *TagsQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TagsQuery) Clone() *TagsQuery {
	...
	return &TagsQuery{
		...
		withServiceTags: tq.withServiceTags.Clone(),
...
func (tq *TagsQuery) WithServiceTags(opts ...func(*ServiceTagQuery)) *TagsQuery {
	query := (&ServiceTagClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withServiceTags = query
	return tq
}
...
func (tq *TagsQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tags, error) {
	...
	if query := tq.withServiceTags; query != nil {
		if err := tq.loadServiceTags(ctx, query, nodes,
			func(n *Tags) { n.Edges.ServiceTags = []*ServiceTag{} },
			func(n *Tags, e *ServiceTag) { n.Edges.ServiceTags = append(n.Edges.ServiceTags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagsQuery) loadServiceTags(ctx context.Context, query *ServiceTagQuery, nodes []*Tags, init func(*Tags), assign func(*Tags, *ServiceTag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Tags)
	for i := range nodes {
	...
	query.Where(predicate.ServiceTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tags.ServiceTagsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.service_tag_tag
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_tag_tag" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_tag_tag" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (tq *TagsQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(tags.Table, tags.Columns, sqlgraph.NewFieldSpec(tags.FieldID, field.TypeUUID))
	_spec.From = tq.sql
